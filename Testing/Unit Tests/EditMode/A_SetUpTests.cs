...
namespace LoganLand.LogansNavmeshExtension.Tests
{
    public class A_SetUpTests
    {
		...
		LNX_NavMeshSurface _jsonGeneratedLnxNavmesh; //note: originally I was going to use this to save the _mesh and triangulation
		...
		public void a1_CreateAndSetUpObjectsInTheScene()
        {
			...
			LNX_NavMeshSurface existingSceneMesh = GameObject.Find(LNX_UnitTestUtilities.Name_ExistingSceneNavmeshGameobject).GetComponent<LNX_NavMeshSurface>();
			...
			Debug.Log($"Now creating scene-generated navmesh...");
			...
			_jsonGeneratedLnxNavmesh = go_serializedNavmesh.AddComponent<LNX_NavMeshSurface>();
		...
		public void b1_Check_That_Bounds_Have_Expected_Values()
		{
			...
			// CENTER-------------------------------------------------------
			Debug.Log($"Checking bounds center. scene generated navmesh bounds center: '{_testGeneratedLnxNavmesh.V_BoundsCenter}'. " +
			...
			UnityEngine.Assertions.Assert.AreApproximatelyEqual( _sceneGeneratedLnxNavmesh_expectedDataModel._Lnx_Navmesh.V_BoundsCenter.x,
				_testGeneratedLnxNavmesh.V_BoundsCenter.x );
			UnityEngine.Assertions.Assert.AreApproximatelyEqual(_sceneGeneratedLnxNavmesh_expectedDataModel._Lnx_Navmesh.V_BoundsCenter.y,
				_testGeneratedLnxNavmesh.V_BoundsCenter.y);
			UnityEngine.Assertions.Assert.AreApproximatelyEqual(_sceneGeneratedLnxNavmesh_expectedDataModel._Lnx_Navmesh.V_BoundsCenter.z,
				_testGeneratedLnxNavmesh.V_BoundsCenter.z);
			// BOUNDS-------------------------------------------------------
			for ( int i = 0; i < 6; i++ )
			{
				Debug.Log($"{i}...");

				UnityEngine.Assertions.Assert.AreApproximatelyEqual( _testGeneratedLnxNavmesh.V_Bounds[i].x, _jsonGeneratedLnxNavmesh.V_Bounds[i].x );
				UnityEngine.Assertions.Assert.AreApproximatelyEqual( _testGeneratedLnxNavmesh.V_Bounds[i].y, _jsonGeneratedLnxNavmesh.V_Bounds[i].y );
				UnityEngine.Assertions.Assert.AreApproximatelyEqual( _testGeneratedLnxNavmesh.V_Bounds[i].z, _jsonGeneratedLnxNavmesh.V_Bounds[i].z );
			}
		}
		#endregion
        ...
        public void C1_Make_Sure_NavMesh_Has_Collection_Lengths_Consistent_With_Triangulation()
        {
            ...
            Assert.AreEqual( _nmTriangulation.areas.Length, _testGeneratedLnxNavmesh.Triangles.Length );
		}
		...
		public void C2_Check_That_Triangle_Indices_Properties_Are_Not_Negative_One()
		{
			...
			for ( int i = 0; i < _testGeneratedLnxNavmesh.Triangles.Length; i++ )
			{
				...
				Assert.Greater( _testGeneratedLnxNavmesh.Triangles[i].Index_inCollection, -1 );
		...
		*/
		...
		public void C5_Check_Triangle_Normals()
		{
			...
			{
				Debug.Log($"i: '{i}'...");
				if( _testGeneratedLnxNavmesh.Triangles[i].v_sampledNormal == Vector3.zero )
				{
		...
		public void D1_Vert_Positions_Are_Kosher()
		{
			...
			for ( int i_tris = 0; i_tris < _testGeneratedLnxNavmesh.Triangles.Length; i_tris++ )
			{
				...
				if( _testGeneratedLnxNavmesh.Triangles[i_tris].Verts[0].V_Position == _testGeneratedLnxNavmesh.Triangles[i_tris].Verts[1].V_Position ||
					_testGeneratedLnxNavmesh.Triangles[i_tris].Verts[0].V_Position == _testGeneratedLnxNavmesh.Triangles[i_tris].Verts[2].V_Position ||
					_testGeneratedLnxNavmesh.Triangles[i_tris].Verts[1].V_Position == _testGeneratedLnxNavmesh.Triangles[i_tris].Verts[2].V_Position
				)
				{
					foundSame = true;
				}
		...
		public void D2_Vert_Angles_Are_Kosher()
		{
			...
			for ( int i_tris = 0; i_tris < _testGeneratedLnxNavmesh.Triangles.Length; i_tris++ )
			{
				...
				{
					Debug.Log($"iterating vert: '{i_verts}', at '{_testGeneratedLnxNavmesh.Triangles[i_tris].Verts[i_verts].V_Position}', with angle '{_testGeneratedLnxNavmesh.Triangles[i_tris].Verts[i_verts].AngleAtBend}'. " +
						$"flattened: '{_testGeneratedLnxNavmesh.Triangles[i_tris].Verts[i_verts].AngleAtBend_flattened}'...");

					Debug.Log($"Asserting that iterated vert has valid angles...");

					Assert.Greater( _testGeneratedLnxNavmesh.Triangles[i_tris].Verts[i_verts].AngleAtBend, 0f );
					Assert.Greater( _testGeneratedLnxNavmesh.Triangles[i_tris].Verts[i_verts].AngleAtBend_flattened, 0f);
				}
			}
		}
		...
		public void D4_Vert_Vismesh_Index_Properties_Are_Within_Bounds()
		{
			...
			for (int i = 0; i < _testGeneratedLnxNavmesh.Triangles.Length; i++)
			{
				...
				for (int i_verts = 0; i_verts < 3; i_verts++)
				{
					...
					Debug.Log($"Verts[{i_verts}].MeshIndex_triangles: '{_testGeneratedLnxNavmesh.Triangles[i].Verts[i_verts].Index_VisMesh_triangles}'");
					Assert.Greater(_testGeneratedLnxNavmesh.Triangles[i].Verts[i_verts].Index_VisMesh_triangles, -1);
					Assert.Less(_testGeneratedLnxNavmesh.Triangles[i].Verts[i_verts].Index_VisMesh_triangles, _testGeneratedLnxNavmesh._VisualizationMesh.triangles.Length);

					Debug.Log($"Verts[{i_verts}].MeshIndex_vertices: '{_testGeneratedLnxNavmesh.Triangles[i].Verts[i_verts].Index_VisMesh_Vertices}'");
					Assert.Greater(_testGeneratedLnxNavmesh.Triangles[i].Verts[i_verts].Index_VisMesh_Vertices, -1);
					Assert.Less(_testGeneratedLnxNavmesh.Triangles[i].Verts[i_verts].Index_VisMesh_Vertices, _testGeneratedLnxNavmesh._VisualizationMesh.vertices.Length);
				}
			}
		}
		...
		public void D5_Vert_SharedVertexCoordinates_Collections_Are_Correct()
		{
			...
			for ( int i_tris = 0; i_tris < _testGeneratedLnxNavmesh.Triangles.Length; i_tris++ )
			{
				for ( int i_verts = 0; i_verts < 3; i_verts++ )
				{
					...
					for ( int i_tris_inner = 0; i_tris_inner < _testGeneratedLnxNavmesh.Triangles.Length; i_tris_inner++ )
					{
						for ( int i_verts_inner = 0; i_verts_inner < 3; i_verts_inner++ )
						{
							LNX_Vertex compareVert = _testGeneratedLnxNavmesh.Triangles[i_tris_inner].Verts[i_verts_inner];
		...
		public void E1_Check_vCross_Gets_Calculated_Correctly()
		{
			...
			Debug.Log($"iterating through '{_testGeneratedLnxNavmesh.Triangles.Length}' triangles...");
			for ( int i_tris = 0; i_tris < _testGeneratedLnxNavmesh.Triangles.Length; i_tris++ )
			{
				Debug.Log($"{nameof(i_tris)}: '{i_tris}'. current normal: '{_testGeneratedLnxNavmesh.Triangles[i_tris].v_sampledNormal}'...");

				if( _testGeneratedLnxNavmesh.Triangles[i_tris].v_sampledNormal == Vector3.zero )
				{
				...
				{
					Debug.Log($"{nameof(i_edge)}: '{i_edge}'...");
					Debug.Log($"Asserting against value: '{_testGeneratedLnxNavmesh.Triangles[i_tris].Edges[i_edge].v_Cross}'...");
					Assert.AreNotEqual( Vector3.zero, _testGeneratedLnxNavmesh.Triangles[i_tris].Edges[i_edge].v_Cross );
		...
		public void F1_Greatest_VisMeshIndex_Is_Same_As_Mesh_Vertices_Array_Length()
		{
			...
			for (int i_triangles = 0; i_triangles < _testGeneratedLnxNavmesh.Triangles.Length; i_triangles++)
			{
				for (int i_verts = 0; i_verts < 3; i_verts++)
				{
					if (_testGeneratedLnxNavmesh.Triangles[i_triangles].Verts[i_verts].Index_VisMesh_Vertices > largestMeshVisIndex_sceneGenerated)
					{
						largestMeshVisIndex_sceneGenerated = _testGeneratedLnxNavmesh.Triangles[i_triangles].Verts[i_verts].Index_VisMesh_Vertices;
					}
				}
			}

			Debug.Log($"End of search. largest vis mesh index was: '{largestMeshVisIndex_sceneGenerated}'...");
			Assert.AreEqual(_testGeneratedLnxNavmesh._VisualizationMesh.vertices.Length - 1, largestMeshVisIndex_sceneGenerated);
			...
			Assert.AreEqual(_jsonGeneratedLnxNavmesh._VisualizationMesh.vertices.Length - 1, largestMeshVisINdex_serialized);
		...
		public void F2_Mesh_Triangles_Array_Is_Expected_Length()
		{
			...
			Debug.Log($"scene mesh's vismesh triangles collection null: '{_testGeneratedLnxNavmesh._VisualizationMesh.triangles == null}'");
			...
			Assert.AreEqual( _sceneGeneratedLnxNavmesh_expectedDataModel._triangulation_areas.Length * 3,
				_testGeneratedLnxNavmesh._VisualizationMesh.triangles.Length );
		}
		...
		public void F3_Mesh_Vertices_Array_Is_Expected_Length()
		{
			...
			Debug.Log($"collection null: '{_testGeneratedLnxNavmesh._VisualizationMesh.vertices == null}'");

			Assert.AreEqual( _sceneGeneratedLnxNavmesh_expectedDataModel._Mesh_Vertices.Length, _testGeneratedLnxNavmesh._VisualizationMesh.vertices.Length );
		}
		...
		*/
		...
		public void F4_All_VisMesh_Verts_Have_Counterpart_At_Position()
		{
			...
			for ( int i_uniqueVert = 0; i_uniqueVert < _testGeneratedLnxNavmesh._VisualizationMesh.vertices.Length; i_uniqueVert++ )
			{
				Debug.Log($"iterating mesh vertex: '{i_uniqueVert}' at pos: '{_testGeneratedLnxNavmesh._VisualizationMesh.vertices[i_uniqueVert]}'...");
				...
				for ( int i_triangles = 0; i_triangles < _testGeneratedLnxNavmesh.Triangles.Length; i_triangles++ )
				{
					...
					for ( int i_vrts = 0; i_vrts < 3; i_vrts++ )
					{
						...
						if( _testGeneratedLnxNavmesh.Triangles[i_triangles].GetVertIndextAtPosition(_testGeneratedLnxNavmesh._VisualizationMesh.vertices[i_uniqueVert]) > -1 )
						{
							haveFound = true;
						}
					}
