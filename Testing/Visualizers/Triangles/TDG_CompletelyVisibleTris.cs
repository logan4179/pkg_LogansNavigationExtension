...
{
    public class TDG_CompletelyVisibleTris : TDG_base
    {
		...
		protected override void OnDrawGizmos()
		{
			...
			DrawStandardFocusTriGizmos( FocusTri, 0.15f, Index_FocusTri.ToString(), Color.magenta, true, 0.03f, false, false );
			...
			for ( int i = 0; i < Indices_knownFullyVisibleTriangles.Length; i++ )
			{
				...
				DrawTriGizmo(_navmesh.Triangles[Indices_knownFullyVisibleTriangles[i]], Color_visibleTris, 0.002f);
				Gizmos.DrawLine
				(
					_navmesh.Triangles[Indices_knownFullyVisibleTriangles[i]].V_Center,
					_navmesh.Triangles[Indices_knownFullyVisibleTriangles[i]].V_Center + (Vector3.up * 0.5f)
				);
			}
			Gizmos.color = oldClr;
			...
			{
				DrawStandardEdgeFocusGizmos( trmnlEdges[i_trmnlEdge], 0.25f, trmnlEdges[i_trmnlEdge].ToString(), Color_obstructEdge );
			}
		}
